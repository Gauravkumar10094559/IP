...
module.exports = app => {
		...
		app.get('/api/landImg', async (req,res) => {
			...
			try{
				let Cricket = await Image.find({ProductTitle:'Cricket'}).limit(4);
				let Music = await Image.find({ProductTitle:'Music'}).limit(4);
				let TVSeries = await Image.find({ProductTitle:'TVSeries'}).limit(4);
				let Gaming= await Image.find({ProductTitle:'Gaming'}).limit(4);

				let response={
					Cricket,Music,TVSeries,Gaming
				};
		...
		try {
				...
				let images = await Image.find({ProductTitle:req.params.prodName});
